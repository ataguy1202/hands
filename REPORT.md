...
The console (`src/session/console.ts`) is small on purpose: a list, a detail page with the screenshot and the buttons, a JSON API under `/api` that a real console or a scripted operator would use. The tests drive a takeover end to end through that API, including a supervisor entering an override in the live session and handing back.
